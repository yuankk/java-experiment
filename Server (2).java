...
public class Server {
	...
	public Server() {
		...
		contentArea.setFont(new Font("宋体",0,16));
	...
	class ClientThread extends Thread {
		...
		public void dispatcherMessage(String message) {
			...
			if (owner.equals("all")) {// 群发
				for (int i = clients.size()-1;(i >= 0) && (!source.equals(clients.get(i).getUser().getName())); i--) {
					clients.get(i).getWriter().println(message + "(多人发送)");
					clients.get(i).getWriter().flush();
				}
			}
			for(int i=0;i<clients.size();i++) {
				if(owner.equals(clients.get(i).getUser().getName())) {
					clients.get(i).getWriter().println(message);
					clients.get(i).getWriter().flush();
			}
				if(source.equals(clients.get(i).getUser().getName())) {
					clients.get(i).getWriter().println(message);
					clients.get(i).getWriter().flush();
			}
			}
